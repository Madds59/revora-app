...
| Information disclosure | Compromised admin account | Full cross-tenant aggregate read via `admin_*` RPCs — this is "full breach" by design once an admin account is compromised. **MFA is now enforced in-codebase, not left to account hygiene**: middleware requires AAL2 — a TOTP factor actually used by *this session* — on every path under `/admin`, and routes a platform admin holding no verified factor to enrollment rather than locking them out (APPSEC-10 Task 7, `apps/web/src/lib/mfa-gate.js`) | **High — the AAL2 requirement is a Next.js ROUTING control only, and this row's scenario is the data layer.** Nothing in the database checks assurance level: `is_super_admin()` (`0009_platform_admins.sql:21-31`) tests only `platform_admins` membership, every `admin_*` function is `grant execute … to authenticated`, and no migration references `aal2`/`assurance`. An attacker holding stolen admin credentials but no second factor therefore still obtains a valid **aal1** token and can call `POST /rest/v1/rpc/admin_list_users` (and every other `admin_*` RPC) straight at PostgREST, bypassing middleware entirely. The gate raises the bar for the admin UI, not for the data. Additionally, a session that has ALREADY satisfied AAL2 and is then stolen (exfiltrated cookies, device malware) retains full admin reach until it expires — the gate authenticates the sign-in, not each later request — and the whole control is only as strong as the hosted project's MFA setting, which `supabase/config.toml` does not govern. RPC-level AAL enforcement is the tracked follow-up; see Known Residuals and Operator Actions in [AUTH_HARDENING.md](AUTH_HARDENING.md) |
