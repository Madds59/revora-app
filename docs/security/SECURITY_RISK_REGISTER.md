...
| APPSEC-09 | P2 | Most dashboard/portal server actions parse `FormData` manually with no schema (zod) validation; RLS is the only backstop | AppSec / Input validation | **Partially fixed — Phase 1 complete** (on branch `security/appsec-09-input-validation-phase-1`). Quotation/job/complaint dashboard actions now validate all external input via a `lib/validation` Zod layer before mutation, with user-safe messages; `addComplaintMessage` `business_id` is now session-derived, not client-trusted. Remaining boundaries (portal customer actions, customers/vehicles/settings/admin/onboarding) are Phase 2+. See [INPUT_VALIDATION_STANDARD.md](INPUT_VALIDATION_STANDARD.md). **Not fully closed** — repository-wide coverage not yet proven. | AppSec Reviewer |
