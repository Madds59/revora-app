...
export default async function BillingPage() {
  ...
  const plans = (planRows ?? []) as BillingPlanCatalogRow[];
  const paymentEvents = (paymentEventRows ?? []) as BillingPaymentEvent[];
  const invoiceList =
    (invoiceResult as PaginatedListResult<BillingInvoiceSummaryRow> | null) ?? {
      rows: [],
      total_count: 0,
    };
  const revenueSummary = (revenueSummaryData ?? null) as BillingRevenueSummary | null;
    ...
    <>
      ...
      <div className="flex flex-col gap-6 p-6">
        ...
        <Card>
          ...
          <CardContent>
            {current && Object.keys(current.entitlements ?? {}).length > 0 ? (
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                {Object.entries(current.entitlements).map(([key, value]) => (
